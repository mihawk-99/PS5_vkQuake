...
## 2026-09-22 — R11 port boot: named padded-row refusal, no presentation

Question: does driver `0e33761` let the actual first frame record and present,
and does a remaining recording refusal reach trace.txt? Added first-result
traces for end/submit/present, retaining every failed result. A host test drives
the real loader wrappers through success and error cases; repeated successes
produce no per-frame spam. The global wrappers are generated by the existing
script, and the window loader covers the device-procedure presentation lookup.

`bash tools/verify.sh` PASS in all five gates. Shader scan completed before
launch. `bash tools/run-title.sh --no-build --watch 900` deployed and verified
identity `6b437103029f1702f911ecb5ea75785133a2880388ea3ec74804cf354ef4d705`,
then launched PPSA99010 PID 195 with the listener already running. Linked
archive: 14,383,172 bytes, SHA-256
`65550cae897ee2fab14224d07b7cf6766e986be21c9e5ba81359b0a0535c75ce`.
The eboot contains the driver's new recording-refusal sentence and the port's
presentation hook, and no old inheritance-framebuffer refusal.

The newest boot compiled 540 shaders successfully and reached
`ps5vk_sampled_image`, which refused set 0 binding 0's 32-texel-wide image
because its stored rows are padded to 256 bytes. `vkEndCommandBuffer -> -13`;
no presentation call occurred. Earlier end/submit success belongs to uploads.
Two `python3 tools/fetch-trace.py` reads were identical, SHA-256
`d3677743b1087467d2db7f349b78c6c8fc43209e288f21976665d7328a6f9b9e`;
the kernel capture identifies PID 195. The harness completed with status 0
and count=0 after the watch, saved trace.txt and left no running title. Its
capture success is not application success. No new exit-signal conclusion is
made from its generic died verdict.

Evidence: `evidence/m2-texture-row-pitch/`; replay command
`python3 tools/evidence.py compare evidence/`: 19 captures, zero failed.
Raw captures remain ignored under klog/. R11's diagnostic criterion is met;
its positive first-frame criterion and M2–M6 remain open. Next driver request:
R12, actual padded texture pitch. No human screen confirmation was requested
because no frame was presented.

## 2026-09-22 — Correction: R12's first host check used the old archive

The initial `tools/check-driver.sh c4_texture` PASS in the driver tree was
reported as testing the new pitch encoding. It only tested the existing R11
archive: check-driver does not rebuild it. The title build's age guard caught
this, and an explicit driver rebuild failed on an undeclared ALIGN macro.
No R12 change reached the console. Per the mission's stop-on-contradiction rule,
the cycle stopped; the candidate is parked with a resumption plan in the driver
at `parked/r12-row-pitch/`. The R11 evidence and its pre-run gates remain valid.
